...
from schemas import (
    ...
    RestaurantInsight, ExtractedDetail, HitlAction,
)
...
llm_qwen = ChatOllama(
    ...
    temperature=0, format="json", num_ctx=4096,
)
...
질문: {query}
""")
    raw = (prompt | llm_qwen | StrOutputParser()).invoke({"query": state["query"]})
...
def searcher(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    raw_results = tool.invoke({"query": f"{state['location']} {state['category']} 맛집 평점 리뷰"})
...
def filter_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    prompt = ChatPromptTemplate.from_template("""
식당 후보에서 품질 낮은 항목을 걸러내세요.
제거 기준: 광고성 콘텐츠, 평점 4.0 미만, 정보 극히 부족

반드시 아래 JSON 형식으로만 답하세요:
{{
  "filtered_candidates": [{{"name":"","score":0.0,"review_count":null,"address":null,"category":null,"price_range":null,"source_url":null,"summary":""}}],
  "needs_retry": false,
  "reason": null
}}
...
""")
    raw = (prompt | llm_qwen | StrOutputParser()).invoke({"candidates": str(raw_candidates)})
    result = parse_structured_output(
        raw, FilterResult,
        FilterResult(
...
내용: {raw_text}
""")
    raw = (prompt | llm_exaone | StrOutputParser()).invoke({
...
""")
    raw = (prompt | llm_gemma | StrOutputParser()).invoke({
